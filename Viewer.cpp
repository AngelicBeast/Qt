...
int RandomInt() {
    ...
    // Initialize a Mersenne Twister generator
    std::mt19937 gen(rd());
    // Define a distribution in the range [0, 255]
...
void Viewer::initialize_buffers()
{
    ...
    // Color buffer
    std::vector<QColor> qtColors;
    for (CGAL::Color cgalColor : colors) {
        QColor qtColor(cgalColor.red(), cgalColor.green(), cgalColor.blue(), cgalColor.alpha());
        qtColors.push_back(qtColor);
    }

    std::vector<float> colorArray;
    colorArray.reserve(qtColors.size() * 4); // Assuming QColor stores 4 components (RGBA)
    for (const QColor& color : qtColors) {
        colorArray.push_back(color.redF() / 255.0f);
        colorArray.push_back(color.greenF() / 255.0f);
        colorArray.push_back(color.blueF() / 255.0f);
        colorArray.push_back(color.alphaF() / 255.0f);
    }

    buffers[3].bind();
    buffers[3].allocate(colorArray.data(), static_cast<int>(colorArray.size() * sizeof(float)));
    int points_colorLocation = rendering_program_points.attributeLocation("color");
    rendering_program_points.enableAttributeArray(points_colorLocation);
    rendering_program_points.setAttributeBuffer(points_colorLocation, GL_FLOAT, 0, 4);
    buffers[3].release();
...
void Viewer::draw()
{
    ...
    color.setRgbF(((float) RandomInt())/255.0f , ((float)RandomInt()) / 255.0f, ((float)RandomInt()) / 255.0f);


    rendering_program_points.setUniformValue(colorLocation, color);
